...
async def show_tariff(message: Message):
    ...
    if lang == "ru":
        text = (
            ...
            f"📅 <b>Дневной</b> — {daily:.2f} USDT (24 часа)\n"
            f"📆 <b>Месячный</b> — {monthly:.2f} USDT (30 дней)\n\n"
            f"💳 Сначала пополните баланс."
        )
    elif lang == "en":
        text = (
            f"📋 <b>Tariff Plans</b>\n\n"
            f"💰 Balance: <b>{balance:.4f} USDT</b>\n\n"
            f"📅 <b>Daily</b> — {daily:.2f} USDT (24 hours)\n"
            f"📆 <b>Monthly</b> — {monthly:.2f} USDT (30 days)\n\n"
    ...
    await message.answer(text, reply_markup=tariff_keyboard(daily, monthly))
...
async def cb_buy_tariff(call: CallbackQuery):
    ...
    if lang == "ru":
        label = "Дневной (1 день)" if tariff_type == "daily" else "Месячный (30 дней)"
        success_text = f"✅ <b>Тариф активирован!</b>\n\n📋 {label}\n💰 {price:.2f} USDT\n📅 До {expires[:10]}"
    elif lang == "en":
        label = "Daily (1 day)" if tariff_type == "daily" else "Monthly (30 days)"
        success_text = f"✅ <b>Tariff activated!</b>\n\n📋 {label}\n💰 {price:.2f} USDT\n📅 Until {expires[:10]}"
    else:
        label = "Kunlik (1 kun)" if tariff_type == "daily" else "Oylik (30 kun)"
        success_text = f"✅ <b>Tarif faollashtirildi!</b>\n\n📋 {label}\n💰 {price:.2f} USDT\n📅 {expires[:10]} gacha"
